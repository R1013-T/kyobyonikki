  ...
  try {
    const insertedDiary = await Promise.all(
      diary.map(async (diary) => {
        return client.sql`
          INSERT INTO diary (emoji, title, content, condition, mental, date, user_id)
          VALUES (${diary.emoji}, ${diary.title}, ${diary.content}, ${diary.condition}, ${diary.mental}, ${diary.date}, ${diary.user_id})
          ON CONFLICT (id) DO NOTHING;
        `
      }),
    )
